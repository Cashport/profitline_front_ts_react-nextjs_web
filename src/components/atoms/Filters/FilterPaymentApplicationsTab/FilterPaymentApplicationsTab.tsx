...
export interface IPaymentApplicationsFilters {
  dates?: string[];
}

interface Props {
  setSelectedFilters: Dispatch<SetStateAction<IPaymentApplicationsFilters>>;
  handleOpenCustomDate?: () => void;
  customDate: string;
  setCustomDate: Dispatch<SetStateAction<string>>;
}

export const FilterPaymentApplicationsTab = ({
  setSelectedFilters,
  ...
  useEffect(() => {
    if (selectedValues.length === 0) {
      return setSelectedFilters({ dates: [] });
    }
    const datesFilters = selectedValues
      .filter((item) => item[0] === "Fechas")
      .map((item) => item[1]);

    setSelectedFilters({ dates: datesFilters });
