...
function getRiskColors(days: number) {
  ...
  if (days <= 7) return { bg: "#EAF6B1", text: "#141414", border: "#DDE78F", label: `${days} días` };
  if (days <= 30) return { bg: "#FDE68A", text: "#141414", border: "#FACC15", label: `${days} días` };
...
export default function ChatConversationList({
  ...
  emptyStateMessage = "No se encontraron tickets",
    ...
    >
      {showMobileSearch && (
        <div className="flex items-center gap-2 px-3 py-3 md:hidden">
          ...
          <Button size="icon" variant="outline" style={{ borderColor: "#DDDDDD" }} onClick={onFilter}>
      ...
      <ScrollArea className="min-h-0 flex-1 pr-2">
        <ul className="px-1">
          {loading ? (
            ...
            <div className="flex items-center justify-center py-8">
              <div className="text-sm text-muted-foreground">{emptyStateMessage}</div>
            </div>
          ) : (
            conversations.map((c) => {
