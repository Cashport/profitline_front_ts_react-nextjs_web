...
import { IBotHistoryDetail } from "../../types/automations";

interface BotHistoryDrawerProps {
  bot: IBotHistoryDetail | null;
  isOpen: boolean;
  onClose: () => void;
}
...
function formatRunTimestamp(run: { date: string; time: string } | null) {
  return run ? `${run.date} · ${run.time}` : "Sin registro";
}

export function BotHistoryDrawer({ bot, isOpen, onClose }: BotHistoryDrawerProps) {
  const statusMeta = bot ? BOT_STATUS_META[bot.estado] : null;
  // El historial viene con la corrida más reciente primero, así que find devuelve la última.
  const lastSuccess = bot?.history.find((run) => run.status === "EXITOSO") ?? null;
  const lastError = bot?.history.find((run) => run.status === "FALLIDO") ?? null;
          ...
          <div className="mb-5 grid grid-cols-1 gap-2.5">
            <SummaryRow
              ...
              value={formatRunTimestamp(
                bot.ultima_ejecucion ? formatLocalDateTimeParts(bot.ultima_ejecucion) : null
              )}
            ...
            <SummaryRow
              ...
              iconColor={BOT_STATUS_META.EXITOSO.color}
              iconBg={BOT_STATUS_META.EXITOSO.bg}
            />
            <SummaryRow
              label="Última ejecución fallida"
              value={formatRunTimestamp(lastError)}
              icon={<XCircle className="h-3.5 w-3.5" />}
              iconColor={BOT_STATUS_META.FALLIDO.color}
              iconBg={BOT_STATUS_META.FALLIDO.bg}
            />
          </div>
                ...
                <div className="flex-1 pb-1">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-medium text-cashport-black">
                      {run.status === "FALLIDO" ? "Ejecución con errores" : "Ejecución exitosa"}
                    </span>
                    <span
