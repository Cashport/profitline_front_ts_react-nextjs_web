...
function NoveltiesChrome({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      <ViewWrapper headerTitle="Novedades" hideHeader className={isDark ? "dark" : ""}>
        {children}
      </ViewWrapper>
    </ConfigProvider>
  );
...
export default function NoveltiesLayout({ children }: { children: React.ReactNode }) {
  return (
    <WalletThemeProvider>
      <NoveltiesChrome>{children}</NoveltiesChrome>
    </WalletThemeProvider>
  );
}
